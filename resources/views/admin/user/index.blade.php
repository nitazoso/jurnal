...
<style>
    ...
    .user-page .delete-modal.show .delete-icon {
    ...
    <section class="activity-card">
        ...
        <form action="{{ route('admin.user.index') }}" method="GET" class="filters">
            <div class="search-box">
                ...
                <input type="text" name="search" placeholder="Cari nama, NIP, atau NISN..." value="{{ request('search') }}">
            </div>

            <div class="role-filters">
                <button type="submit" name="role" value="" class="role-filter {{ !request('role') ? 'active' : '' }}">Semua</button>
                <button type="submit" name="role" value="Guru" class="role-filter {{ request('role') === 'Guru' ? 'active' : '' }}">Guru</button>
                <button type="submit" name="role" value="Staff Piket" class="role-filter {{ request('role') === 'Staff Piket' ? 'active' : '' }}">Staff</button>
            </div>
            ...
            <table>
                ...
                <tbody>
                    @forelse($users as $user)
                        <tr>
                            <td class="user-cell"><strong>{{ $user->nama_user }}</strong></td>
                            ...
                            <td class="user-actions">
                                <a href="{{ route('admin.user.edit', $user->id_user) }}" class="icon-action edit" title="Edit user">
                                    <span class="material-symbols-outlined">edit</span>
                                </a>
                                <button type="button" class="icon-action delete" title="Hapus user" onclick="openDeleteModal(
                                    '{{ route('admin.user.destroy', $user->id_user) }}',
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="empty-row">Belum ada data user.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            @if($users->hasPages())
                <div class="pagination">
                    @if($users->onFirstPage())
                        <span class="page disabled"><span class="material-symbols-outlined">chevron_left</span></span>
                    @else
                        <a href="{{ $users->previousPageUrl() }}" class="page"><span class="material-symbols-outlined">chevron_left</span></a>
                    @endif
                    ...
                    @for($page = 1; $page <= $last; $page++)
                        @if($page === 1 || $page === $last || abs($page - $current) <= 1)
                            ...
                            @else
                                <a href="{{ $users->url($page) }}" class="page">{{ $page }}</a>
                            @endif
                    ...
                    @if($users->hasMorePages())
                        <a href="{{ $users->nextPageUrl() }}" class="page"><span class="material-symbols-outlined">chevron_right</span></a>
                    @else
                        <span class="page disabled"><span class="material-symbols-outlined">chevron_right</span></span>
                    @endif
                </div>
            @endif
        </div>
    ...
    <div id="deleteModal" class="delete-modal">
        <div class="delete-modal-box">
            <div class="delete-modal-header">
                <div class="delete-icon"><span class="material-symbols-outlined">warning</span></div>
                <div class="delete-modal-title">
                    <h3>Hapus User?</h3>
                    <p>Apakah Anda yakin ingin menghapus user ini?<br>Tindakan ini tidak dapat dibatalkan.</p>
                </div>
            </div>

            <div class="delete-info">
                <span id="deleteName" class="delete-info-name">Nama User</span>
                <div class="delete-info-detail">
                    <span>Username: <strong id="deleteUsername">username</strong></span>
                    <span class="dot">•</span>
                    <span>Role: <strong id="deleteRole">Guru</strong></span>
                </div>
            </div>

            <div class="delete-warning">
                <span class="material-symbols-outlined">info</span>
                <span>User yang dihapus tidak dapat dikembalikan dan seluruh akses akun tersebut akan dinonaktifkan.</span>
            </div>
            ...
            <div class="delete-modal-footer">
                ...
                <form id="deleteForm" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn-delete-confirm">Hapus User</button>
                </form>
            </div>
        </div>
    </div>
